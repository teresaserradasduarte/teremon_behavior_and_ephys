...
neurons_params.sp.meta_ap = meta_ap;
neurons_params.sp.meta_lf = meta_lf;
...
ExtraGood = sp.cegs(filterQualityUnits);

% Cell type: is complex spike?
if (isfield(sp,'potential_CS'))
    if ~isempty(sp.potential_CS)
        pCS = sp.potential_CS(filterQualityUnits);
    else
        pCS = [];
    end
end
...
for iUnit = 1:nOfUnits
    ...
    temp_neurons(iUnit).st_all = all_st(cluSpikeBool);
