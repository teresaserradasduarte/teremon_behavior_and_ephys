...
for s=1:size(all_sess,1)
    ...
    for r =1:size(all_reg,1)
        ...
        reach1Idx = find(bhv.cat_reach_inVec==1);
        hLRC_idx = zeros(length(reach1Idx),1);
        hLRC_idx(ismember(reach1Idx,left_r1_idx))=1;
        hLRC_idx(ismember(reach1Idx,center_r1_idx))=2;
        hLRC_idx(ismember(reach1Idx,right_r1_idx))=3;
        figure
        plot(squeeze(bhv.reaches_inVec_px(:,1,left_r1_idx)),'color',behavior.colors.left_color); hold on
        plot(squeeze(bhv.reaches_inVec_px(:,1,center_r1_idx)),'color',behavior.colors.center_color); hold on
        plot(squeeze(bhv.reaches_inVec_px(:,1,right_r1_idx)),'color',behavior.colors.right_color); hold off


        %% FR averaged per condition
        ...
        figure
        plot(squeeze(bhv.reaches_inVec_px(:,1,left_hit_idx)),'color',behavior.colors.left_color); hold on
        plot(squeeze(bhv.reaches_inVec_px(:,1,center_hit_idx)),'color',behavior.colors.center_color); hold on
        plot(squeeze(bhv.reaches_inVec_px(:,1,right_hit_idx)),'color',behavior.colors.right_color); hold off
        ...
        if strcmp(mouse_info.paw_pref,'R')
            FR_rDCnD = cat(1,r1R_FR_mean,r1C_FR_mean,r1L_FR_mean);
            FR_hDCnD = cat(1,hR_FR_mean,hC_FR_mean,hL_FR_mean);
        elseif strcmp(mouse_info.paw_pref,'L')
            FR_rDCnD = cat(1,r1L_FR_mean,r1C_FR_mean,r1R_FR_mean);
            FR_hDCnD = cat(1,hL_FR_mean,hC_FR_mean,hR_FR_mean);
        end
