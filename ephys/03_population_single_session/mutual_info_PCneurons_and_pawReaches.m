...
win_size = .05;
start_win = 0;
stop_win = start_win+win_size;

% Entropy
hist_nbins = 20;

% entropy paw
flag_win_paw = time_paw >= start_win & time_paw <= stop_win;
paw_win_hist = mean(paw_r1(flag_win_paw,:),1);
[pPaw, pawHist_edges] = histcounts(paw_win_hist,hist_nbins,...
...
neu_win_hist = mean(neu_r1(flag_win_neu,:,i),1);
[pNeu, neuHist_edges] = histcounts(neu_win_hist,hist_nbins,...
    'Normalization','probability');
entropy_neu = -sum(pNeu.*log2(pNeu),'omitnan');

% Entropy joint
[pJointPawNeu, ~, ~] = histcounts2(paw_win_hist,neu_win_hist,hist_nbins,...
    'Normalization','probability');
entropy_joint = -sum(pJointPawNeu(:).*log2(pJointPawNeu(:)),'omitnan');

MI_paw_neu = entropy_paw + entropy_neu - entropy_joint;



%%
save_entropy = strcat(save_path,filesep,'pool_trials');
if ~exist(save_poll_trials,"dir"), mkdir(save_poll_trials); end


figure
subplot(131)
histogram(paw_win_hist,pawHist_edges,'Normalization','probability')
title(sprintf('%s%.2f','H(paw) = ',entropy_paw))
xlabel('paw pc1'); ylabel('probability')
    set(gca,axeOpt{:})

subplot(132)
histogram(neu_win_hist,neuHist_edges,'Normalization','probability')
title({ ...
...
subplot(133)
histogram2(paw_win_hist,neu_win_hist,pawHist_edges,neuHist_edges,'Normalization','probability')
...
set(gcf,'position',[2094         281        1594         531])
