...
if  exist(strcat(char(folders.mat),filesep,'session_raw_data_paw',paw_pref,'.mat'),'file')~=2
    ...
    % Check max size to define the mat length
    if (siz(1)>7*frame_rate*60), start_trials = 2;
    else, start_trials = 1; end   
    ...
    save(strcat(char(folders.mat),filesep,'session_raw_data_paw',paw_pref,'.mat'),...
        'folders','resnet', 'paw_pref', 'start_trials',...
    ...
    load(strcat(char(folders.mat),filesep,'session_raw_data_paw',paw_pref,'.mat'),...
        ...
        'water','water_mm','siz','max_siz','n_max_points','resnet'...
        )
    disp('loaded tracked features data')
    if  ~exist("start_trials","var")
        if siz(1)>max_siz, start_trials=2;
        else, start_trials=1; end
    end
...
siz_sess = sum(siz(start_trials:end));
ppDom_paw_sess = [];
ppNonDom_paw_sess = [];
...
siz_no1(1) = max_siz;
% loop across trials
for i = start_trials:n_trials
    if i==1
        speed_DomPaw(1:siz_no1(i),:,i) = speed_DomPaw_sess(1:siz_no1(i),:,i);
    else
        if start_trials>1
            speed_DomPaw(1:siz_no1(i),:,i) = speed_DomPaw_sess(sum(siz_no1(start_trials:(i-1)))+1:sum(siz_no1(start_trials:i)),:);
        else
            speed_DomPaw(1:siz(i),:,i) = speed_DomPaw_sess(sum(siz(start_trials:(i-1)))+1:sum(siz(start_trials:i)),:);
        end
...
if strcmp(mouse,'3_Toblerone')
    extra_frames=40;
else
    extra_frames=10;
end
nan_min = 40;
water_intermitent_max = 15;
...
flag_hit_from_harp = zeros(n_trials,1);
...
for tt = start_trials:n_trials
    ...
    jmp_cumsum_nan = find(diff(cumsum_nan)==1);
    jmp_cumsum_nan_small = jmp_cumsum_nan(diff(jmp_cumsum_nan)<=1);
    check_water_tracking_interm = length(jmp_cumsum_nan)-length(jmp_cumsum_nan_small);
    ...
    elseif max(cumsum_nan)<nan_min
        ...
        if check_water_tracking_interm>water_intermitent_max % CHECK THIS 
            last_water = water_reached_frame(tt);
            flag_hit_from_harp(tt) = 1; 
        else
    ...
    if nreaches_trial == 0
        ...
        for r_ind = 1:nreaches_trial
            ...
            % PAW NOT DISPLACEDE ENOUGH TO BE A REACH
            reach_displacement_short = reach(max_reach,1)-reach(rising_start,1)<min_length_drink;
            ...
            elseif  (start_from_stop_resting || start_forward_mov_resting)
                ...
                if (~(start_from_stop_lifted || start_forward_mov_lifted) && ...
                        mean_reach_until_reach>threshold_drink && reach_displacement_short)
                    which_cat = 4;
                else
                    which_cat = 2;
                end
...
trials.flag_hit_from_harp = flag_hit_from_harp;
% Trial paramers
