function plot_init_raster_and_psth(eg_neurons,i,bin_edges,win_interest,figProp,trialsIdx)
    ...
    spike_times = eg_neurons(i).st_init(trialsIdx)';
    trials_idx_scat = cell2mat(arrayfun(@(i) i * ones(1, numel(spike_times{i})),...
    1:length(spike_times), 'UniformOutput', false));

    nexttile;
    plot(bin_edges,mean(eg_neurons(i).FR_init(:,trialsIdx),2,'omitnan'),'Color',figProp.rcl_clr,LineWidth=2)
    hold off
    xlim(win_interest)
